...
| 6-9 | répéter la même offre | `market` : la catégorie la plus demandée devient ton produit |
...
`--lang en|fr|pt` change la langue. Attention : en `fr` et `pt`, seule la
structure est traduite — les puces métier restent en anglais. Relis avant
d'envoyer (ce que tu dois faire de toute façon).
